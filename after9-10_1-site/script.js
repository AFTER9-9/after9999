  ...
  function buildPreorderCards(){
    ...
    PRODUCTS.forEach(function(p){
      ...
      card.innerHTML =
        ...
        '<div class="name">' + p.name + '</div>' +
        '<div class="meta"><span>' + p.meta + '</span><b>GHS ' + p.price + '</b></div>' +
        '<button class="preorder-buy" type="button">Pre-order — GHS ' + p.price + '</button>';
      card.querySelector('.preorder-buy').addEventListener('click', function(){
        openCheckout({ name: p.name + ' (Pre-order)', price: p.price });
      });
  ...
  function refreshDropState(){
    ...
    if(soldOut){
      ...
      if(buyBtn){ buyBtn.textContent = 'Buy Now — GHS ' + LE_PRODUCT.price; buyBtn.style.display = 'inline-flex'; }
    } else {
      // Pre-order enabled: purchasing is open even before the drop unlocks/reveals.
      if(buyBtn){ buyBtn.textContent = 'Pre-order — GHS ' + LE_PRODUCT.price; buyBtn.style.display = 'inline-flex'; }
    }
